...
/**
 * This class contains the bases of RPC connecting from one component to another component, for
 * example the RPC connection from ShuffleWorker to ShuffleManager. This {@code
...
public abstract class RegisteredRpcConnection<
    ...
    /** The target component Address, for example the ShuffleManager Address. */
    private final String targetAddress;

    /**
     * Execution context to be used to execute the on complete action of the
     * ShuffleManagerRegistration.
     */
    ...
    /**
     * This method generate a specific Registration, for example shuffle worker Registration at the
     * shuffle manager.
     */
    ...
    private RetryingRegistration<F, G, S> createNewRegistration() {
        ...
        CompletableFuture<Pair<G, S>> future = newRegistration.getFuture();

        future.whenCompleteAsync(
                (Pair<G, S> result, Throwable failure) -> {
                    if (failure != null) {
                        ...
                        targetGateway = result.getLeft();
                        onRegistrationSuccess(result.getRight());
                    }
                },
                executor);
