...
package com.alibaba.flink.shuffle.e2e.utils;
...
public class CommonTestUtils {
    ...
    /** Create a temporary log4j configuration for the test. */
    public static File createTemporaryLogFile() throws IOException {
        File logFile = File.createTempFile(FileUtils.getRandomFilename(""), "-log");
    ...
    public static void printLog4jDebugConfig(File file) throws IOException {
        try (PrintWriter writer = new PrintWriter(new FileWriter(file))) {
            ...
            writer.println("rootLogger.appenderRef.file.ref = MainAppender");
            writer.println("appender.main.name = MainAppender");
            writer.println("appender.main.type = RollingFile");
            writer.println("appender.main.append = true");
            writer.println("appender.main.fileName = ${sys:log.file}");
            writer.println("appender.main.filePattern = ${sys:log.file}.%i");
            writer.println("appender.main.layout.type = PatternLayout");
            writer.println(
                    "appender.main.layout.pattern = %d{yyyy-MM-dd HH:mm:ss,SSS} %-5p %c{1} - %m%n");
            writer.println("appender.main.policies.type = Policies");
            writer.println("appender.main.policies.size.type = SizeBasedTriggeringPolicy");
            writer.println("appender.main.policies.size.size = 100MB");
            writer.println("appender.main.policies.startup.type = OnStartupTriggeringPolicy");
            writer.println("appender.main.strategy.type = DefaultRolloverStrategy");
            writer.println("appender.main.strategy.max = ${env:MAX_LOG_FILE_NUMBER:-10}");
    ...
    public static void delayCheck(RunnableWithException runnable, Deadline timeout, String errorMsg)
            throws Exception {
        waitUntilCondition(
                () -> {
                    try {
