...
public class LocalMapPartitionFileReader {
    ...
    /** Buffer for reading buffer length to memory. */
    private final ByteBuffer lengthBuffer = IOUtils.allocateHeaderBuffer();
    ...
    public LocalMapPartitionFileReader(
            int startPartitionIndex, int endPartitionIndex, LocalMapPartitionFile partitionFile) {
    ...
    public boolean readBuffer(ByteBuffer buffer) throws Exception {
        ...
        try {
            ...
            currentPartitionRemainingBytes -=
                    IOUtils.readBuffer(dataFileChannel, lengthBuffer, buffer);
